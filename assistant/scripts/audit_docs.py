...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Audita e normaliza documentação do K.A.O.S"
    )
    parser.add_argument(
        "--mode",
        choices=["audit", "fix"],
    ...
    if args.mode == "audit":
        ...
        print(
            f"Nota de Saúde Geral:    {results['health_metrics']['documentation_health']:.1f}%"
        )
        print("===================================================================\n")
        ...
        print("------------------------|----------|----------|-------")
        print(
            f"Coverage Score          | {results['before']['coverage_score']}%     | {results['after']['coverage_score']}%     | {'[OK]' if results['after']['coverage_score'] >= 90 else '[ERRO]'}"
        )
        print(
            f"Completeness Score      | {results['before']['completeness_score']}%     | {results['after']['completeness_score']}%     | {'[OK]' if results['after']['completeness_score'] >= 85 else '[ERRO]'}"
        )
        print(
            f"Consistency Score       | {results['before']['consistency_score']}%     | {results['after']['consistency_score']}%     | {'[OK]' if results['after']['consistency_score'] >= 90 else '[ERRO]'}"
        )
        print(
            f"Link Health Score       | {results['before']['link_health_score']}%     | {results['after']['link_health_score']}%     | {'[OK]' if results['after']['link_health_score'] >= 95 else '[ERRO]'}"
        )
        print(
            f"KIRL Integrity Score    | {results['before']['kirl_integrity_score']}%     | {results['after']['kirl_integrity_score']}%     | {'[OK]' if results['after']['kirl_integrity_score'] >= 95 else '[ERRO]'}"
        )
        print(
            f"Architecture Alignment  | {results['before']['arch_alignment_score']}%     | {results['after']['arch_alignment_score']}%     | {'[OK]' if results['after']['arch_alignment_score'] >= 90 else '[ERRO]'}"
        )
        print(
            f"Drift Score             | {results['before']['drift_score']}%     | {results['after']['drift_score']}%     | {'[OK]' if results['after']['drift_score'] <= 10 else '[ERRO]'}"
        )
        print(
            f"Documentation Health    | {results['before']['documentation_health']}%     | {results['after']['documentation_health']}%     | {'[OK]' if results['after']['documentation_health'] >= 90 else '[ERRO]'}"
        )
        print("--------------------------------------------------------------------")
