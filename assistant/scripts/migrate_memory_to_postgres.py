...
async def migrate(dry_run: bool = False):
    ...
    for user_dir in users_dir.iterdir():
        ...
        for conv in conversations:
            if dry_run:
                logger.info(
                    f"[DRY-RUN] Migraria: user={user_id} session={conv['session_id']}"
                )
                total_migrated += 1
                continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Migra memoria.md do Obsidian para PostgreSQL"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Apenas simula, não grava no banco"
    )
