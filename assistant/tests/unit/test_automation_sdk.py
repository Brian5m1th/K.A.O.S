...
async def test_automation_service_trigger():
    ...
    mock_provider.trigger_workflow = AsyncMock(return_value={"success": True, "execution_id": "exec_abc"})
    
    with patch("app.core.automation_bus.AutomationBus.get_provider", return_value=mock_provider):
...
async def test_automation_service_emit():
    with patch("app.core.automation_bus.AutomationBus.publish", new_callable=AsyncMock) as mock_pub:
...
async def test_automation_service_wait_for():
    ...
    with patch("app.core.automation_sdk.async_session_factory", return_value=lambda: mock_factory_instance):
