...
def test_secret_manager_missing_key(monkeypatch):
    ...
    with pytest.raises(ValueError, match="KAOS_SECRET_KEY environment variable is required"):
...
def test_secret_manager_rotation():
    ...
    assert new_fernet.decrypt(rotated["github_token"].encode("utf-8")).decode("utf-8") == "token_val"
    assert new_fernet.decrypt(rotated["slack_webhook"].encode("utf-8")).decode("utf-8") == "slack_val"
