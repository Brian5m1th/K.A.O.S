...
class TestKnowledgeGraph:
    def test_infer_type(self):
        assert KnowledgeGraphBuilder._infer_type("desktop/src/shared/lib/stores/useThemeStore.ts") == "store"
        assert KnowledgeGraphBuilder._infer_type("desktop/src/pages/welcome/index.tsx") == "page"
        assert KnowledgeGraphBuilder._infer_type("assistant/app/tools/calculator.py") == "tool"
        assert KnowledgeGraphBuilder._infer_type("assistant/app/observability/event_bus.py") == "event"
        assert KnowledgeGraphBuilder._infer_type("unknown_ref") == "unknown"

    @patch("app.ai.vault_analyzer.knowledge_graph.RuntimePathResolver.knowledge_graph_path")
    def test_load_and_persist(self, mock_path, tmp_path):
        temp_file = tmp_path / "knowledge-graph.json"
        mock_path.return_value = temp_file

        kg = KnowledgeGraph(
            nodes=[{"id": "node-1", "title": "Node 1", "type": "sdd", "source": "vault"}],
    ...
    def test_update_file_vault(self, mock_scan_single, mock_project_root, mock_path, tmp_path):
        temp_file = tmp_path / "knowledge-graph.json"
        mock_path.return_value = temp_file
        ...
        kg = KnowledgeGraph(
            nodes=[
                {"id": "old-node", "title": "Old Node", "type": "sdd", "source": "vault", "path": "docs/sdd/old.md"}
            ],
            edges=[],
        )
        KnowledgeGraphBuilder._persist(kg)
    ...
    def test_delete_file(self, mock_project_root, mock_path, tmp_path):
        ...
        kg = KnowledgeGraph(
            nodes=[
                {"id": "node-to-delete", "title": "Delete Me", "type": "sdd", "source": "vault", "path": "docs/sdd/delete.md"}
            ],
            edges=[
                {"source": "node-to-delete", "target": "other-node", "relation": "depends_on"}
            ],
        )
        KnowledgeGraphBuilder._persist(kg)
