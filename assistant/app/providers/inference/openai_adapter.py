...
class OpenAIAdapter(InferencePort):
    ...
    async def invoke(self, request: InferenceRequest) -> InferenceResult:
        ...
        return InferenceResult(
            content=result.content, provider="openai", model=provider.model_name
        )
...
class GeminiAdapter(InferencePort):
    ...
    async def invoke(self, request: InferenceRequest) -> InferenceResult:
        ...
        provider = GeminiProvider(
            model=request.model or "gemini-pro", api_key=settings.GEMINI_API_KEY
        )
        result = await provider.ainvoke(_to_langchain(request.messages))
        return InferenceResult(
            content=result.content, provider="gemini", model=provider.model_name
        )
...
class ClaudeAdapter(InferencePort):
    ...
    async def invoke(self, request: InferenceRequest) -> InferenceResult:
        ...
        provider = ClaudeProvider(
            model=request.model or "claude-sonnet", api_key=settings.ANTHROPIC_API_KEY
        )
        result = await provider.ainvoke(_to_langchain(request.messages))
        return InferenceResult(
            content=result.content, provider="claude", model=provider.model_name
        )
