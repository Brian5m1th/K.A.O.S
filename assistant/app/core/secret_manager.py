...
class SecretManager:
    ...
    def _get_fernet(cls) -> Fernet:
        if cls._fernet is None:
            ...
            if not key:
                raise ValueError(
                    "KAOS_SECRET_KEY environment variable is required and must not be empty."
                )
    ...
    def decrypt(cls, cipher_text: str) -> str:
        ...
        try:
            ...
            raise ValueError(
                "Failed to decrypt secret data. The key may be incorrect or corrupted."
            )

    @classmethod
    def rotate_key(
        cls, old_key: str, new_key: str, data: dict[str, str]
    ) -> dict[str, str]:
        """Decrypts dictionary values with old_key and re-encrypts with new_key."""
        try:
            old_fernet = Fernet(
                old_key.encode("utf-8") if isinstance(old_key, str) else old_key
            )
            new_fernet = Fernet(
                new_key.encode("utf-8") if isinstance(new_key, str) else new_key
            )
        except Exception as e:
