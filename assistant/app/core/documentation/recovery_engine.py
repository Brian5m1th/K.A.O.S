...
class RecoveryEngine:
    ...
    def fix_encoding(cls, content: str) -> tuple[str, bool]:
        ...
        for bad, good in cls.ENCODING_REPLACEMENTS.items():
            if (
                bad in new_content
                or bad.lower() in new_content
                or bad.upper() in new_content
            ):
                # Substituição case-insensitive
                new_content = re.sub(
                    re.escape(bad), good, new_content, flags=re.IGNORECASE
                )
                modified = True
    ...
    def inject_frontmatter(
        cls, path: Path, content: str, default_meta: dict
    ) -> tuple[str, bool]:
    ...
    def fix_wikilinks(cls, content: str, file_path: Path) -> tuple[str, bool]:
        ...
        def replace_link(match) -> str:
            ...
            if not replacement:
                split_links = cls._split_combined_link(
                    target, valid_stems, redirections
                )
                if split_links:
    ...
    @classmethod
    def _split_combined_link(
        cls, target: str, valid_stems: set[str], redirections: dict
    ) -> list[str]:
        target_norm = cls._normalize_target(target)
    ...
    @classmethod
    def generate_stub(
        cls, feature_id: str, feature_name: str, phase: str, code_refs: list[str]
    ) -> Path:
