...
class GraphValidator:
    ...
    def validate_graph(cls, all_docs: dict[str, dict[str, Any]]) -> dict[str, Any]:
        ...
        orphans = []
        connections = {
            name: {"incoming": set(), "outgoing": set()} for name in doc_names
        }
        ...
        if total_nodes > 0:
            graph_connectivity = (
                (total_nodes - disconnected_count) / total_nodes
            ) * 100.0
        else:
            ...
            for p in parents:
                ...
                if not p_match:
                    hierarchy_issues.append(
                        f"Nota '{name}' refere pai inexistente '{p}'"
                    )
    ...
    def _find_match(cls, target_name: str, doc_names: set[str]) -> str | None:
        ...
        for name in doc_names:
            ...
            if name_stem.lower().strip() == target_lower:
                return name
            name_stem_norm = (
                name_stem.lower().replace("-", " ").replace("_", " ").strip()
            )
