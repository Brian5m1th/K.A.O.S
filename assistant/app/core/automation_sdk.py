...
from app.models.automation_registry import AutomationExecution, AutomationWorkflow
from app.core.automation_bus import AutomationBus


class AutomationService:
    @classmethod
    async def trigger(cls, workflow_id_or_path: str, payload: dict[str, Any]) -> dict[str, Any]:
        """
    ...
    @classmethod
    async def wait_for(cls, execution_id: str, timeout_seconds: int = 120) -> dict[str, Any]:
        """
        ...
        logger.info(f"[AutomationSDK] Waiting for execution '{execution_id}' (timeout={timeout_seconds}s)")
        
        while True:
            # Check elapsed time
            elapsed = (datetime.now(timezone.utc) - start_time).total_seconds()
            if elapsed > timeout_seconds:
                logger.warning(f"[AutomationSDK] Wait timeout for execution '{execution_id}'")
                return {"success": False, "status": "timeout", "error": "Execution wait timeout exceeded"}
            ...
            async with factory() as session:
                ...
                if execution:
                    if execution.status in ("success", "failed"):
                        logger.info(f"[AutomationSDK] Execution '{execution_id}' finished with status: {execution.status}")
                        return {
                        ...
                        logger.debug(f"[AutomationSDK] Execution '{execution_id}' is still in state: {execution.status}")
                else:
                    logger.debug(f"[AutomationSDK] Execution '{execution_id}' not found in DB yet, continuing poll.")
