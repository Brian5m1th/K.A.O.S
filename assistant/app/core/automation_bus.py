    ...
    async def auto_import_workflows(cls) -> None:
        ...
        for attempt in range(retries):
            try:
                ...
                pass
            logger.debug(f"[AutomationBus] Connection attempt {attempt+1}/{retries} failed. Retrying in {delay}s...")
            await asyncio.sleep(delay)
            delay *= 2.0
            
        if not online:
            logger.warning("[AutomationBus] Workflow engine is offline. Skipping default workflows auto-import.")
            return
            
        logger.info("[AutomationBus] Workflow engine is online. Running template auto-import...")
        
        workflows_dir = Path("data/workflows")
        if not workflows_dir.exists():
            logger.info("[AutomationBus] Workflows directory 'data/workflows' does not exist.")
            return
        ...
        for path in workflows_dir.glob("*.json"):
            try:
                ...
                async with factory() as session:
                    stmt = select(AutomationWorkflow).where(AutomationWorkflow.name == wf_name)
                    result = await session.execute(stmt)
                    existing = result.scalar_one_or_none()
                    
                    if existing:
                        logger.debug(f"[AutomationBus] Workflow '{wf_name}' already registered in local DB.")
                        continue
                        
                    # Import to external engine
                    logger.info(f"[AutomationBus] Auto-importing default workflow '{wf_name}'...")
                    ...
                    logger.info(f"[AutomationBus] Registered auto-imported workflow '{wf_name}' with ID {remote_id}")
            except Exception as e:
                logger.error(f"[AutomationBus] Failed to auto-import '{path.name}': {e}")

    @classmethod
    async def publish(cls, event_name: str, data: dict[str, Any], depth: int = 0) -> None:
        """
        ...
        logger.debug(f"[AutomationBus] Event '{event_name}' enqueued (trace_id={trace_id}, depth={depth})")
    ...
    async def _consume_queue(cls) -> None:
        ...
        while True:
            ...
            try:
                ...
                logger.debug(f"[AutomationBus] Processing event '{event_name}' (trace_id={trace_id})")
