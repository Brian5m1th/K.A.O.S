...
def validate_features_index() -> CheckResult:
    ...
    invalid = [
        f["id"]
        for f in features
        if f.get("status") not in ("done", "in_progress", "planned")
    ]
    if invalid:
        return CheckResult(
            "features_index",
...
async def validate_heatmap_api() -> CheckResult:
    ...
    return CheckResult(
        ...
        False,
        "registry",
        {"score": data.get("score", 0), "level": data.get("level", "unknown")},
    )
    ...
    return CheckResult(
        ...
        "source",
        {
            "installed": list(
                k
                for k in deps
                if k in ("reactflow", "@xyflow/react", "cytoscape", "d3")
            )
        },
    )
...
async def validate_heatmap_data() -> CheckResult:
    ...
    if score == 0:
        return CheckResult(
            ...
            False,
            "registry",
            {"error": "Heatmap score is 0 (no real data)", "score": score},
        )
    return CheckResult(
        "heatmap_data",
        "pass",
...
CHECKS: list[CheckDefinition] = [
    ...
    CheckDefinition(
        "heatmap_api", "V4", "semantic", False, "registry", ["features_index"]
    ),
    CheckDefinition(
        "graph_summary",
        "V5",
    ...
    CheckDefinition(
        "features_index", "V9b", "structural", True, "registry", ["snapshot_json"]
    ),
...
def compute_fingerprint() -> str:
    ...
    if snap:
        ...
        parts.append(
            json.dumps(
                {
                    "coverage": snap.get("coverage", 0),
                    "graph_nodes": gs.get("totalNodes", 0),
                },
                sort_keys=True,
            )
        )
