...
async def opencode_status():
    ...
    refs = sorted(p.stem for p in (OPCODE_DIR / "references").glob("*.md")) if (OPCODE_DIR / "references").exists() else []
    ...
    return {"configured": True, "path": str(OPCODE_DIR), "agents": agents, "references": refs, "rules": rules, "skills": skills, "tools": tools, "total": len(agents)+len(refs)+len(rules)+len(skills)+len(tools)}
...
async def list_agents():
    ...
    for f in sorted(d.glob("*.md")):
        ...
        agents.append({"id": f.stem, "name": c.split("\n")[0].lstrip("# ") if c else f.stem, "file": f.name})
